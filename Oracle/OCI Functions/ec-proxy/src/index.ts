...
fdk.handle(function (input: Proxy.RequestInput) {
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof (path) !== "string") {
			const error = "Engagement Cloud endpoint not set. Use '/' for root path.";
			console.error(error);
			reject({ "error": error });
		}

		if (typeof (method) !== "string") {
			const error = "Method for Engagement Cloud endpoint not set.";
			console.error(error);
			reject({ "error": error });
		}
		...
		if (typeof (headers) !== "object") {
			...
			headers["Content-Length"] = payload !== null ? payload!.length : 0;
