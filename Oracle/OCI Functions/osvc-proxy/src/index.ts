...
fdk.handle(function (input: Proxy.RequestInput) {
	console.debug("Service Cloud Proxy Called");
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof (path) !== "string") {
			const error = "Service Cloud endpoint not set. Use '/' for root path.";
			console.error(error);
			reject({ "error": error });
		}

		if (typeof (method) !== "string") {
			const error = "Method for Service Cloud endpoint not set.";
			console.error(error);
			reject({ "error": error });
		}
		...
		headers["OSvC-CREST-Application-Context"] = "AR Accelerator";
