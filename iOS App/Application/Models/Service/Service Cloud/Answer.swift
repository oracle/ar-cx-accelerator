    ...
    */
    func xmlToType<T: Decodable>(object: T.Type) -> T? {
        guard let xml = self.xml, let xmlData = xml.data(using: .utf8) else {
            os_log(.error, "No XML data attached to answer")
    ...
    */
    func getPDFFile(completion: @escaping (_ pdf: PDFDocument?) -> ()) {
        ...
        if url.absoluteString.contains("http") {
            ...
            ((UIApplication.shared.delegate as? AppDelegate)?.integrationBroker as? IntegrationBroker)?.asyncHttpRequest(session: session, request: request, completion: { (result) in
                switch result {
